...
Ostatnia aktualizacja: 2026-07-01 (Faza 4 / Unit 5 domknięty + Unit 6 ukończony — parametry + telemetria + panel)
...
### Review Fazy 3 (2026-07-01, commit f019b33) — ✅ CZYSTE
Raport: `review-faza-3.md`. Walidacja: host **425/425 PASS** (+12), `idf.py build` (esp32s3) PASS (exit 0). Severity gate CZYSTE: **P1=0, P2=0, P3=1**.
- **Wszystkie 9 inwariantów safety szczelne z realną mocą wyroczni:** (1) failsafe-precedence — goto/GPS/IMU/comms wyłącznie w `resolve_spot_lock` (ARMED-only), zero dotknięcia `rc_valid`/`sm_inputs`/`sm_step`; `test_goto_rc_loss_failsafe_wins` wchodzi w stan przeciekający (drifted+aligned=WOULD thrust), asertuje ESC=1500+servo=center+OFF. (2) Cykl życia latcha — clear TYLKO override/CH3-preempt (`goto_latch_should_clear`), NIGDY comms/GPS-loss; pauza-vs-abort rozróżnione asercjami `goto_latch_clear==false/true`. (3) Domena zegara — `sensor_is_fresh`/`sensor_freshness_stamp` obie w `now_ms()`, wrap-safe. (4) comms_fresh co cykl w `apply_goto_inputs`. (5) hard clamp SI-3 (`esc_forward_max_us=3000`→2000). (6) schemat 6→7 legalny (blob 61/65→63/67, round-trip non-default, anchor update ≠ weakening). (7) zero test-weakeningu (diff addytywny +12). (8) Pure⊥HAL (`loop_step.h`/`spot_lock.h` grep-clean, `apply_goto_inputs` cienki). (9) single-writer (s_goto_* zapisywane wyłącznie w tasku pętli, sekwencyjnie w cyklu).
- **P3 nit (1):** `loop_step.c:271-279` `goto_latch_should_clear` re-derywuje warunki priorytetu (`!sticks_neutral || ch3_on`) współdzielone z `spot_lock_step`; dopuszczalna świadoma duplikacja (coding-rules §11), warto komentarz o synchronizacji.
- **Odchylenie od planu (uzasadnione):** zakres Unit 5 (`goto_comms_timeout_ms` end-to-end + testy) przeniesiony do Fazy 3, bo watchdog potrzebuje realnego timeoutu zamiast magic number. Pozostały ślad Unit 5 (dedykowany test 409-w-ARMED) + telemetria/panel (Unit 6) = Faza 4, zgodne z planem (NIE braki).
- **Wniosek:** jądro bezpieczeństwa feature'u strukturalnie szczelne. Faza 3 gotowa do kontynuacji (Faza 4).

### Faza 4 — Parametry + telemetria + panel (Unit 5 domknięty + Unit 6) — ukończona 2026-07-01
- **Unit 5 (domknięcie):** parametr `goto_comms_timeout_ms` był już dostarczony end-to-end w Fazie 3 (model+bump v6→v7, ranges MIN200/MAX5000/DEF1500, defaults, validate, params_json, blob_codec + testy validate/blob). W Fazie 4 dołożono JEDYNIE brakujący ślad: dedykowany `test_armed_rejects_goto_comms_timeout_write` w `test_params_decide.c` (409-w-ARMED). Analogiczny do `test_armed_rejects_new_spot_lock_param_write`: SI-6 jest bramką na stan (nie na pole POST), `fields_valid=true` izoluje bramkę → 409/API_ERR_NOT_DISARMED. Zero duplikacji wcześniejszej pracy.
- **Unit 6 (telemetria + panel):** kontrakt WS dla aplikacji iOS. Pola snapshotu (`control_loop_snapshot`): `goto_state` (0/1/2, ≠0 tylko gdy SRC_GOTO — CH3 hold czyta off), `goto_target_lat_e7/lon_e7`, `goto_err_m`, `goto_bearing_deg10`, `goto_arrived`, `app_link_fresh`. Populacja goto-specyficzna wyekstrahowana do `loop_telemetry.goto_*` (`loop_step.h`/`loop_step.c`): liczone z `sl` (spot_lock_outputs) TYLKO gdy `state->spot_lock.target_source==SRC_GOTO`, inaczej off/0 — dzięki temu ten sam silnik spot-lock nie myli holdu CH3 z goto. `publish_snapshot` mapuje cel z `s_goto_lat/lon_e7`, `app_link_fresh` z `in->comms_fresh`. `ws_telemetry.c::snapshot_to_json` serializuje ints/bools (`%u/%d/%s`); bufor json 512→640 (7 nowych pól, 2× int32). Front-end: karta „Goto (app nav)" w `web/index.html` + render w `web/app.js` (`GOTO_STATE_NAMES`).
- **Testy (host):** 3 nowe w `test_loop_step.c` — `test_goto_telemetry_populates_while_active` (goto view = mirror spot_lock_* gdy ACTIVE), `_arrived_within_deadband` (arrived=true na celu), `_reads_off_under_ch3_hold` (MOC WYROCZNI: CH3 hold ⟹ goto off/0 mimo spot_lock ACTIVE — bez rozgraniczenia target_source test FAILuje). +1 w `test_params_decide.c`. Host: **429/429 PASS** (+4 vs 425 baseline Fazy 3, zero regresji). `idf.py build` (esp32s3): PASS (37% partycji wolne).
- **Decyzja:** `snapshot_to_json` to HAL (linkuje esp_http_server/esp_log, funkcja static) — nie host-linkowalny. Host-test pokrywa warstwę populacji `loop_telemetry.goto_*` (realna logika decyzyjna); serializacja JSON weryfikowana przez `idf.py build` + wizualnie w panelu (E2E). `goto_state` jako pole GOTO-SPECYFICZNE (nie współdzielone z `spot_lock_state`) — app dostaje jednoznaczny sygnał „czy goto aktywne", odseparowany od CH3-holdu (parytet API iOS).
- **Odłożone do `known-issues.md §4d`:** [E2E] panel/aplikacja (off→active, malejący błąd, paused na link, arrived), realna nawigacja, pauza/wznowienie na link, CH3-preempt w terenie, dryf — brak przeglądarki/hardware/aplikacji iOS.

### Review Fazy 4 (2026-07-01, commit fcc8743) — ✅ CZYSTE
Raport: `review-faza-4.md`. Walidacja: host **429/429 PASS** (+4), `idf.py build` (esp32s3) PASS (bin 0xf1ce0, 37% wolne). Severity gate CZYSTE: **P1=0, P2=0, P3=2**.
- **Werdykt pkt 1–6 pozytywny:** (1) goto telemetria gated na `target_source==SRC_GOTO` — `test_goto_telemetry_reads_off_under_ch3_hold` ma moc wyroczni (asertuje spot_lock ACTIVE ORAZ goto OFF/0 pod CH3-hold; naiwny mirror bez bramki źródła FAILuje). (2) serializacja ints/bools only (`%u/%d/%s`, zero floatów); bufor 640 z zapasem dla realnych wartości, guard `len>=sizeof(json)` przechwytuje ewentualny truncation bez przepełnienia. (3) nazwy `goto_*` = stabilny kontrakt iOS, spójne z `spot_lock_*`/`gps_*`; enum GOTO_STATE_NAMES panelu == enum firmware. (4) 409-w-ARMED izoluje bramkę SI-6 (`fields_valid=true`), 4 asercje. (5) zero test-weakeningu (+4 addytywne). (6) panel bez XSS — `setText` używa `textContent`, podziały display-side, wire pozostaje int.
- **P3 nit (2):** (a) `snapshot_to_json` bufor 640 dobrany pod typowe wartości — hipotetyczne maksimum wszystkich pól uint razem >640, nieosiągalne w praktyce i guard chroni (snprintf bounded); opcjonalny bump ~768 lub komentarz. (b) `test_armed_rejects_goto_comms_timeout_write` near-duplikat wariantu spot_lock (`params_decide_write` field-agnostyczny) — wartość dokumentacyjna, dopuszczalne §11.
- **Wniosek:** Faza 4 gotowa do domknięcia zadania. Pozostają domknięcia zadaniowe: dokumentacja kontraktu API iOS + rozważenie `/dev-compound`.
